                ...
                <Box sx={{ p: 2 }}>
                    ...
                    <form onSubmit={handleSubmit}>
                        <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center', mb: 2 }}>
                            ...
                            >
                                {loading ? <CircularProgress size={24} color="inherit" /> : 'Choose File'}
                                <input
                                    type="file"
                                    accept="image/*"
                        ...
                        <Box sx={{ p: 2 }}>
                            {faceData && faceData.length > 0 ? (
                                faceData.map((face, index) => (
                                    <Box key={index} sx={{ mt: 2, bgcolor: '#f3e5f5', p: 2, borderRadius: '4px' }}>
                                        <Typography variant="subtitle1" gutterBottom>Face {index + 1}</Typography>
                                        {renderFaceData(face)}
                                    </Box>
                                ))
                            ) : (
